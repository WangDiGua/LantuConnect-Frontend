...
| 2026-03-19 | 无次级侧栏时主区水平留白收紧：`LayoutChromeContext` + 概览/快捷/占位/个人中心/AI 助手等；`document.documentElement` 字号与外观「字号」同步，全站 rem 联动；`MgmtPageShell` 面包屑 + 主区 `overflow-y-auto`；侧栏 Logo 文案「智能体接入平台」；次级侧栏顶栏面包屑「智能体接入平台 › 当前主菜单」 |
| 2026-03-19 | 系统配置三页 `views/systemConfig/*`（模型配置 / 限流策略 / 审计日志）接入 `SystemConfigModule`；产品品牌为「智能体接入平台」，`index.html` 标题/描述与 `public/favicon.svg`；主内容区切换动画 key 含 `activeAgentView` 与 `selectedAgentId`，过渡 `0.28s`；用户/角色列表↔表单使用 `AnimatePresence`；原生下拉统一 `utils/formFieldClasses.ts`（`rounded-xl`），Daisy `select` 补充 `rounded-xl` |
...
- **需要弹出选项列表与主题一致时**（圆角、边框、深浅色与触发器对齐），必须使用 **`LantuSelect`**（`src/components/common/LantuSelect.tsx`）或基于其封装的 **`FilterSelect`**。原生 `<select>` 的展开层由系统绘制，无法用项目 CSS 统一风格。
- **`nativeSelectTriggerClass(theme)`** 与 **`LantuSelect`** 触发按钮共用底色/边框/圆角；**`nativeSelectClass(theme)`** 仅保留给极少数遗留场景，新代码勿再使用原生 `<select>` 作主要下拉。
- 历史说明：原生 `<select>` 若仍存在，须带 **`nativeSelectClass(theme)`**、`appearance-none` 与 **`index.css` 内 `.lantu-select`** 背景箭头。
- 与 `join` 组合时：左侧用 **`LantuSelect`**，`className="join-item w-32 shrink-0 min-w-0"`，`triggerClassName` 含 `rounded-l-xl rounded-r-none border-r-0`；右侧 input 使用 `rounded-l-none rounded-r-xl`。
