<div align="center">
  <h1>🌐 LantuConnect - 智慧校园智能体接入平台</h1>
  <p>企业级前端应用 | Agent Gateway Platform</p>
  <p>
...
**LantuConnect（兰智通）** 是一个面向高校的智能体接入与管理平台，集成了智能体（Agent）、智能应用、数据集、工作流等核心功能，为师生提供便捷、高效、安全的AI服务接入能力。
...
VITE_DEFAULT_THEME_COLOR=indigo
```
...
开发环境支持完整的Mock数据系统，无需后端即可开发前端功能。

### 使用方式

1. 设置 `VITE_USE_MOCK=true`
2. Mock数据在 `src/lib/mock/handlers/` 目录
3. 所有API请求自动拦截并返回Mock数据

### 切换真实API

设置 `VITE_USE_MOCK=false`，并配置 `VITE_API_PROXY_TARGET` 指向后端服务。
...
5. 添加Mock处理器（`src/lib/mock/handlers/`）
...
<div align="center">
  ...
  <p>© 2026 LantuConnect. All rights reserved.</p>
</div>
