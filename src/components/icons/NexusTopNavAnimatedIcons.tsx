...
export interface NexusNavSvgProps {
  ...
  isDark?: boolean;
  className?: string;
  /** 为 true 时播放 SMIL 动效；顶栏仅在当前选中项为 true，默认静止 */
  ...
  return (
    ...
    >
      ...
      {motionActive ? (
        ...
        <circle cx="32" cy="32" r="22" stroke={`url(#${gid})`} strokeWidth="1.5" opacity="0.35" />
      )}
...
/** Skills 中心 */
export const NexusNavIconSkills: React.FC<NexusNavSvgProps> = ({ className, motionActive = false }) => {
  const u = useNavSvgUid();
...
/** MCP 广场 */
export const NexusNavIconMcp: React.FC<NexusNavSvgProps> = ({ isDark = false, className, motionActive = false }) => {
  const u = useNavSvgUid();
  const gid = `${u}-mcp-grad`;
  const nodeFill = isDark ? '#0f172a' : 'white';
  const hub = motionActive ? (
...
/** 数据集 */
export const NexusNavIconDatasets: React.FC<NexusNavSvgProps> = ({ className, motionActive = false }) => {
  const u = useNavSvgUid();
...
/** Agent 广场 */
export const NexusNavIconAgent: React.FC<NexusNavSvgProps> = ({ isDark = false, className, motionActive = false }) => {
  const stroke = isDark ? '#cbd5e1' : '#1e293b';
  const star = motionActive ? (
    ...
    <path d="M32 26L33.5 30.5L38 32L33.5 33.5L32 38L30.5 33.5L26 32L30.5 30.5L32 26Z" fill="#8b5cf6" opacity={0.95} />
  );
  const dot = motionActive ? (
    <circle cx="42" cy="26" r="1.5" fill="#22d3ee">
      <animate attributeName="opacity" values="0;1;0" dur="1.5s" repeatCount="indefinite" />
    </circle>
  ) : (
    <circle cx="42" cy="26" r="1.5" fill="#22d3ee" opacity={0.35} />
  );
  const scanLine = motionActive ? (
    ...
    <line x1="18" y1="32" x2="46" y2="32" stroke="#3b82f6" strokeWidth="1" strokeDasharray="2 4" opacity="0.22" />
  );
  return (
    <svg width="24" height="24" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} aria-hidden>
...
/** 应用集 */
export const NexusNavIconAppSet: React.FC<NexusNavSvgProps> = ({ isDark = false, className, motionActive = false }) => {
  const ring = isDark ? '#334155' : '#f1f5f9';
  const b1 = motionActive ? (
...
/** 用户顶栏六个一级入口的 SVG（`motionActive` 为 true 时 SMIL 动效；`prefers-reduced-motion` 时父级改用 Lucide） */
