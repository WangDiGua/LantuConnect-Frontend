...
function ensureRequiredHeaders(config: InternalAxiosRequestConfig): void {
  ...
  if (path === '/v2/capabilities/import' && config.method?.toLowerCase() === 'post' && !hasUserId) {
    throw new ApiException({ code: 1001, status: 400, message: '调用 POST /v2/capabilities/import 必须提供 X-User-Id' });
  }
  if (path === '/v2/capabilities' && config.method?.toLowerCase() === 'post' && !hasUserId) {
    throw new ApiException({ code: 1001, status: 400, message: '调用 POST /v2/capabilities 必须提供 X-User-Id' });
  }
  if (/^\/v2\/capabilities\/[^/]+\/(resolve|invoke|tool-session)$/.test(path) && !hasUserId && !hasApiKey) {
    throw new ApiException({ code: 1001, status: 400, message: '调用 Capability V2 执行接口必须提供 X-User-Id 或 X-Api-Key' });
  }
...
function mapErrorMessage(status: number, code?: number, fallback?: string): string {
  ...
  if (code === 3002) return fallback || '今日调用次数已达上限，请明日再试';
  if (code === 3003) return fallback || '本月调用次数已达上限，请下月再试';
  if (code === 3004) return fallback || '服务熔断中，请稍后重试';
  if (code === 3005) return fallback || '当前调用已受平台策略限制，请联系管理员处理';
