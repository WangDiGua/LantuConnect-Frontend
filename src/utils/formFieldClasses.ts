...
export function nativeSelectTriggerClass(theme: Theme): string {
  const d = theme === 'dark';
  return `w-full rounded-xl border px-3 py-2.5 text-sm min-h-[2.5rem] outline-none transition-all duration-200 motion-reduce:transition-none shadow-sm focus-visible:outline-none ${
    d
...
export function nativeInputClass(theme: Theme): string {
  const d = theme === 'dark';
  return `w-full rounded-xl border px-4 py-2.5 text-sm min-h-[2.5rem] box-border outline-none transition-all duration-200 motion-reduce:transition-none shadow-sm focus-visible:outline-none ${
    d
