            ...
            return (
              <React.Fragment key={step.id}>
                <div className="flex flex-col items-center gap-2">
                  ...
                  <span className={`text-xs font-bold text-center max-w-[80px] ${isActive || isCompleted ? 'text-indigo-600 dark:text-indigo-400' : textMuted(theme)}`}>
                    {step.label}
                  </span>
                </div>
    ...
    return (
      <BentoCard theme={theme} padding="sm" className="mt-6">
        ...
        <div className="space-y-2 text-xs">
          {[
            { label: '名称', value: formData.name || '未填写' },
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
        {AGENT_TEMPLATES.map((template) => (
          <BentoCard
            ...
            hover
            padding="sm"
            className={`cursor-pointer ${selectedTemplate === template.id ? 'ring-2 ring-indigo-500/40 border-indigo-500/30' : ''}`}
            onClick={() => applyTemplate(template)}
          >
            <div className="flex items-center gap-3 mb-2">
              <div className={`p-2 rounded-xl ${selectedTemplate === template.id ? 'bg-indigo-600 text-white' : isDark ? 'bg-white/10 text-slate-400' : 'bg-slate-100 text-slate-500'}`}>
    ...
    >
      ...
      <div className="flex justify-between pt-4">
        <button
          ...
          className={`${btnGhost(theme)} gap-2 ${showPreview ? 'text-indigo-500' : ''}`}
        >
          <Eye size={16} />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {types.map((type) => (
            <BentoCard
              ...
              hover
              padding="sm"
              className={`cursor-pointer ${formData.type === type.id ? 'ring-2 ring-indigo-500/40 border-indigo-500/30' : ''}`}
              onClick={() => setFormData({ ...formData, type: type.id })}
            >
              <div className="flex items-center gap-3 mb-2">
                <div className={`p-2 rounded-xl ${formData.type === type.id ? 'bg-indigo-600 text-white' : isDark ? 'bg-white/10 text-slate-400' : 'bg-slate-100 text-slate-500'}`}>
                  <type.icon size={20} />
                </div>
    ...
    >
      ...
      <BentoCard theme={theme} padding="sm">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className={`text-sm font-semibold ${textPrimary(theme)}`}>快速配置</span>
        </div>
        ...
        <div className="space-y-5">
          <div>
            ...
            <div className="join w-full">
              <select
                className={`${selectCls} join-item w-32 shrink-0 rounded-l-xl rounded-r-none border-r-0`}
          ...
          <div className={`border-t pt-5 ${isDark ? 'border-white/[0.06]' : 'border-slate-100'}`}>
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                <label className={`${labelCls} mb-1.5 block`}>响应超时 (秒)</label>
                <input
                  type="number"
                  className={inputCls}
  ...
  const renderSubmitting = () => (
    <div className="flex flex-col items-center justify-center py-10 text-center">
      <motion.div
        ...
        className="w-12 h-12 rounded-full border-4 border-indigo-500/20 border-t-indigo-500 mb-6"
      />
  ...
  return (
    <div className={`flex-1 flex flex-col min-h-0 overflow-hidden ${pageBg(theme)}`}>
      <div className={`shrink-0 z-20 border-b px-4 sm:px-6 py-4 flex items-center gap-4 backdrop-blur-xl ${
        ...
        <h2 className={`text-lg font-bold ${textPrimary(theme)}`}>{isEditMode ? '编辑 Agent' : '创建新 Agent'}</h2>
      </div>
