...
import {
  pageBg, btnPrimary, btnSecondary, btnGhost, textPrimary, textSecondary, textMuted,
...
const STATUS_CONFIG: Record<VersionStatus, { label: string; light: string; dark: string; dot: string }> = {
  ...
  testing:  { label: '测试中', light: 'bg-blue-50 text-blue-700 ring-1 ring-blue-200/60',   dark: 'bg-blue-500/10 text-blue-400 ring-1 ring-blue-500/20',   dot: 'bg-blue-500' },
  released: { label: '已发布', light: 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200/60', dark: 'bg-emerald-500/10 text-emerald-400 ring-1 ring-emerald-500/20', dot: 'bg-emerald-500' },
  rollback: { label: '已回滚', light: 'bg-orange-50 text-orange-700 ring-1 ring-orange-200/60', dark: 'bg-orange-500/10 text-orange-400 ring-1 ring-orange-500/20', dot: 'bg-orange-500' },
};
...
export const AgentVersionPage: React.FC<Props> = ({ theme }) => {
  ...
  const handleCreateVersion = () => {
    ...
    versionService.create(selectedAgentId as number, { version: newVersion.startsWith('v') ? newVersion : `v${newVersion}`, changelog: newChangelog || '(无变更说明)' })
  ...
  return (
    <div className={`flex-1 flex flex-col min-h-0 overflow-hidden transition-colors duration-300 ${pageBg(theme)}`}>
      <div className="flex-1 min-h-0 overflow-y-auto px-2 sm:px-3 lg:px-4 py-2 sm:py-3 space-y-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className={`p-2 rounded-xl ${isDark ? 'bg-blue-500/15' : 'bg-blue-50'}`}><GitBranch size={20} className={isDark ? 'text-blue-400' : 'text-blue-600'} /></div>
            <div>
              <h1 className={`text-lg font-bold ${textPrimary(theme)}`}>版本管理</h1>
              <p className={`text-xs ${textMuted(theme)}`}>Agent 版本迭代、发布与回滚</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="relative">
              <select value={selectedAgentId} onChange={(e) => { setSelectedAgentId(e.target.value ? Number(e.target.value) : ''); setCompareMode(false); setCompareA(''); setCompareB(''); }} className={nativeSelectClass(theme)}>
            ...
            {selectedAgentId && (
              <>
                ...
                <button type="button" onClick={() => setShowCreateModal(true)} className={btnPrimary}><Plus size={14} /> 创建新版本</button>
              </>
            )}
          </div>
        </div>
        ...
        {!selectedAgentId ? (
          <BentoCard theme={theme} className="p-12 text-center">
            ...
            <p className={`text-lg font-medium ${textSecondary(theme)}`}>请先选择一个 Agent</p>
          ...
          <>
            ...
            <BentoCard theme={theme} padding="sm" className="flex items-center justify-between px-5 py-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-indigo-600 flex items-center justify-center text-white text-lg font-bold">{selectedAgent?.displayName.charAt(0)}</div>
                <div>
                    ...
                    <div key={v.id} className="flex gap-4">
                      ...
                      <div className={`flex-1 ${isLast ? 'pb-2' : 'pb-6'}`}>
                        <div className={`rounded-xl border p-4 transition-colors ${isDark ? 'border-white/[0.04] hover:border-white/10 bg-white/[0.02]' : 'border-slate-100 hover:border-slate-200 bg-slate-50/50'}`}>
                          ...
                          <div className={`flex items-center gap-1.5 text-xs ${textMuted(theme)}`}><Clock size={12} /> {v.createTime}</div>
                        </div>
                      </div>
                    </div>
                  );
            ...
            {compareMode && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                <BentoCard theme={theme}>
                  <h3 className={`font-bold mb-4 flex items-center gap-2 ${textPrimary(theme)}`}><ArrowRight size={18} className="text-indigo-500" /> 版本对比</h3>
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <select value={compareA} onChange={(e) => setCompareA(e.target.value ? Number(e.target.value) : '')} className={nativeSelectClass(theme)}>
                      <option value="">选择版本 A…</option>
                      {versions.map((v) => <option key={v.id} value={v.id}>{v.version} ({STATUS_CONFIG[v.status].label})</option>)}
                    </select>
                    <select value={compareB} onChange={(e) => setCompareB(e.target.value ? Number(e.target.value) : '')} className={nativeSelectClass(theme)}>
                        ...
                        return (
                          <div key={vd.id} className={`rounded-xl border p-4 ${isDark ? 'border-white/[0.04] bg-white/[0.02]' : 'border-slate-100 bg-slate-50/50'}`}>
                            ...
                            <div className="space-y-2">
                              ...
                              <div><label className={`text-[11px] font-medium ${textMuted(theme)}`}>时间</label><p className={`text-sm font-mono mt-0.5 ${textSecondary(theme)}`}>{vd.createTime}</p></div>
                            </div>
                          </div>
                        );
                      })}
      ...
      <Modal open={showCreateModal} onClose={() => setShowCreateModal(false)} title="创建新版本" theme={theme} size="md" footer={<><button type="button" className={btnSecondary(theme)} onClick={() => setShowCreateModal(false)}>取消</button><button type="button" className={btnPrimary} onClick={handleCreateVersion} disabled={!newVersion.trim()}><Plus size={14} /> 创建版本</button></>}>
        <div className="space-y-4">
          <div><label className={`text-xs font-semibold block mb-1 ${textSecondary(theme)}`}>版本号</label><input type="text" value={newVersion} onChange={(e) => setNewVersion(e.target.value)} placeholder="例如: v2.2.0" className={nativeInputClass(theme)} /></div>
          ...
          <div className={`flex items-start gap-2 p-3 rounded-xl text-xs ${isDark ? 'bg-indigo-500/10 text-indigo-400' : 'bg-indigo-50 text-indigo-600'}`}>
