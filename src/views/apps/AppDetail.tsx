...
export const AppDetail: React.FC<AppDetailProps> = ({ appId, theme, onBack, showMessage }) => {
  ...
  if (error || !app) return <div className={`flex-1 flex flex-col min-h-0 ${pageBg(theme)}`}><PageError error={error} onRetry={fetchApp} /></div>;
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 300, damping: 30 }} className={`${bentoCard(theme)} p-6`}>
            ...
            <div className="grid grid-cols-2 gap-5">
              {[
                ...
                { label: '状态', value: statusLabel(app.status as DomainStatus) },
                { label: '是否公开', value: app.isPublic ? '是' : '否' },
              ].map((item) => (
                <div key={item.label}>
        ...
        <div className="space-y-4">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 300, damping: 30, delay: 0.1 }} className={`${bentoCard(theme)} p-6`}>
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-xs"><span className={textMuted(theme)}>创建时间</span><span className={`font-mono ${textSecondary(theme)}`}>{app.createTime}</span></div>
              <div className="flex justify-between text-xs"><span className={textMuted(theme)}>最后更新</span><span className={`font-mono ${textSecondary(theme)}`}>{app.updateTime}</span></div>
            </div>
          </motion.div>
        </div>
      </div>
