...
export const DataReportsPage: React.FC<DataReportsPageProps> = ({ theme, fontSize: _fontSize }) => {
  ...
  const inputClass = `px-3 py-2 rounded-xl text-sm border outline-none transition-colors ${
    isDark ? 'bg-white/[0.04] border-white/[0.06] text-white focus:border-indigo-500/40' : 'bg-white border-slate-200 text-slate-900 focus:border-indigo-400'
  }`;
        ...
        >
          <div className="flex items-center gap-3">
            <div className={`p-2.5 rounded-xl ${isDark ? 'bg-indigo-500/15' : 'bg-indigo-50'}`}>
              <BarChart3 size={22} className={isDark ? 'text-indigo-400' : 'text-indigo-600'} />
            </div>
            <div>
              <h1 className={`text-lg font-bold ${textPrimary(theme)}`}>数据报表</h1>
              <p className={`text-xs ${textSecondary(theme)}`}>平台调用统计、用户活跃分析与趋势报表</p>
            </div>
          </div>
        </motion.div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-5 shrink-0">
              {[
                ...
                { label: 'Top Agent 数', value: String(topAgents.length), icon: <Bot size={16} />, glow: 'rose' as const },
                  ...
                  <div className="flex items-end gap-2 h-48">
                    {bars.map((bar, i) => {
                      ...
                      return (
                        ...
                        >
                          <span className={`text-[10px] font-medium tabular-nums ${textMuted(theme)}`}>
                          ...
                          />
                          <span className={`text-[10px] ${textMuted(theme)}`}>{bar.label}</span>
                        </motion.div>
                      );
                    })}
                  </div>
                      ...
                      <div key={agent.name} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3 mt-1">
                            ...
                            <span className={`text-[10px] tabular-nums shrink-0 ${textMuted(theme)}`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-5">
              ...
              >
                <BentoCard theme={theme} padding="sm" className="!p-0 overflow-hidden">
                  <div className={`px-5 py-3.5 border-b ${isDark ? 'border-white/[0.06]' : 'border-slate-100'}`}>
                    <h3 className="text-sm font-semibold uppercase tracking-wider text-slate-400">Agent 成功率</h3>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                      <thead>
                        <tr>
                          <th className={tableHeadCell(theme)}>Agent 名称</th>
