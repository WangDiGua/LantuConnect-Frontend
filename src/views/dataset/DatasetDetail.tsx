...
const SOURCE_LABEL: Record<string, string> = { department: '部门数据', knowledge: '知识库', third_party: '第三方' };
const DATA_TYPE_LABEL: Record<string, string> = { document: '文档', structured: '结构化', image: '图片', audio: '音频', video: '视频', mixed: '混合' };
...
export const DatasetDetail: React.FC<DatasetDetailProps> = ({ datasetId, theme, onBack, showMessage }) => {
  ...
  if (error || !dataset) return <div className={`flex-1 flex flex-col min-h-0 ${pageBg(theme)}`}><PageError error={error} onRetry={fetchDataset} /></div>;
      ...
      <div className={`shrink-0 z-20 border-b px-4 sm:px-6 py-4 flex items-center justify-between ${isDark ? 'border-white/[0.06]' : 'border-slate-100'}`}>
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="flex items-center gap-3 min-w-0">
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shrink-0 ${isDark ? 'bg-violet-500/15' : 'bg-violet-50'}`}>📊</div>
            <div>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar px-2 sm:px-3 lg:px-4 py-3 grid grid-cols-1 lg:grid-cols-3 gap-4 content-start">
        <div className="lg:col-span-2 space-y-4">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 300, damping: 30 }} className={`${bentoCard(theme)} p-6`}>
            ...
            <div className="grid grid-cols-2 gap-5">
              {[
                { label: '显示名称', value: dataset.displayName }, { label: '数据集标识', value: dataset.datasetName, mono: true },
                ...
                { label: '是否公开', value: dataset.isPublic ? '是' : '否' },
              ].map((item) => (
                <div key={item.label}>
                  <label className={`text-xs block mb-1 ${textMuted(theme)}`}>{item.label}</label>
                  {item.badge ? (
                    <span className={`inline-flex items-center px-2 py-1 rounded-lg text-xs font-semibold ${isDark ? 'bg-violet-500/10 text-violet-400' : 'bg-violet-50 text-violet-600'}`}>{item.value}</span>
                  ) : (
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 300, damping: 30, delay: 0.05 }} className={`${bentoCard(theme)} p-6`}>
            ...
            <div className="space-y-3">
              <div className={`flex items-start gap-3 p-3 rounded-xl ${isDark ? 'bg-white/[0.02]' : 'bg-slate-50'} border ${isDark ? 'border-white/[0.06]' : 'border-slate-100'}`}>
                ...
                <div><p className={`text-sm font-bold ${textPrimary(theme)}`}>记录数</p><p className={`text-xs ${textMuted(theme)}`}>{formatCount(dataset.recordCount)} 条</p></div>
              </div>
              <div className={`flex items-start gap-3 p-3 rounded-xl ${isDark ? 'bg-white/[0.02]' : 'bg-slate-50'} border ${isDark ? 'border-white/[0.06]' : 'border-slate-100'}`}>
        ...
        <div className="space-y-4">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 300, damping: 30, delay: 0.1 }} className={`${bentoCard(theme)} p-6`}>
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-xs"><span className={textMuted(theme)}>创建时间</span><span className={`font-mono ${textSecondary(theme)}`}>{dataset.createTime}</span></div>
              <div className="flex justify-between text-xs"><span className={textMuted(theme)}>最后更新</span><span className={`font-mono ${textSecondary(theme)}`}>{dataset.updateTime}</span></div>
            </div>
          </motion.div>
        </div>
      </div>

      <ConfirmDialog open={deleteOpen} title="删除数据集" message={`确定要删除「${dataset.displayName}」吗？此操作不可撤销。`} confirmText="删除" variant="danger" loading={deleting} onConfirm={handleDelete} onCancel={() => setDeleteOpen(false)} />
    </div>
  );
};
