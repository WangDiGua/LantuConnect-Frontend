...
const API_CATEGORIES: ApiCategory[] = [
  ...
  { id: 'resolve-invoke', label: '解析与调用', endpoints: [
    ...
    { method: 'POST', path: '/invoke', description: '统一调用入口。**X-Api-Key 必填**。还须 Key scope 含 invoke（或 *）；**已发布**资源可被具备 scope 的调用方使用。**skill** 仅支持托管形态：走 **POST /invoke**（`resourceType=skill`），由平台执行 LLM 路由；不再支持 zip/制品包形态的网关 invoke。**绑定展开**：`agent` 且存在 `agent_depends_mcp`，或托管 `skill` 且被 `mcp_depends_skill` 引用时，网关在转发前把绑定 MCP 的 `tools/list` 聚合写入 **`payload._lantu.bindingExpansion`**（`entry` / `openAiTools` / `routes` / `warnings`）；仅 invoke `mcp` 不反向拉起 Agent；各 MCP 须具备 invoke scope（否则记入 `warnings`）。配置项 `lantu.gateway.binding-expansion` 可关闭。', params: [{ name: 'resourceType', type: 'string', required: true, description: '资源类型' }, { name: 'resourceId', type: 'string', required: true, description: '资源 ID' }, { name: 'payload', type: 'object', required: false, description: '业务输入；展开时合并 _lantu.bindingExpansion（保留 payload._lantu 其它子键）' }], responseExample: JSON.stringify({ code: 0, message: 'ok', data: { requestId: 'req_1', traceId: 'tr_1', resourceType: 'agent', resourceId: '1', statusCode: 200, status: 'success', latencyMs: 124, body: '{\"answer\":\"ok\"}' } }, null, 2) },
            ...
            <div className="flex-1 min-w-0">
              <article className="mx-auto max-w-3xl px-4 py-6 sm:px-8 sm:py-10">
                ...
                <section id="doc-keys" className="mt-14 space-y-4">
                  ...
                  {prosePara(theme, (
                    <>
                      在「个人设置 → 密钥管理」页面创建。响应中的 <span className="font-mono">secretPlain</span>（或等价字段）<strong className={textPrimary(theme)}>仅出现一次</strong>，请立即安全保存。列表只有掩码与前缀。撤销/删除后无法找回明文，只能重建 Key 并重配集成环境变量与调用方配置。
                    </>
                  ))}
                  {prosePara(theme, (
                    <>
