...
const API_CATEGORIES: ApiCategory[] = [
  { id: 'capability-v2', label: 'Capability V2（主推）', endpoints: [
    { method: 'POST', path: '/v2/capabilities/import', description: '智能导入入口。提交 URL、OpenAPI、MCP 配置、模型端点、JSON 或 prompt 描述后，后端自动识别为 skill / mcp / agent，并补全最小可用默认值。须提供 X-User-Id。', params: [{ name: 'source', type: 'string', required: true, description: '待识别的导入源' }], responseExample: JSON.stringify({ code: 0, message: 'ok', data: { detectedType: 'agent', runtimeMode: 'remote_agent', displayName: 'OpenAI Responses', resourceCode: 'openai-responses', defaults: { model: 'gpt-5.4-mini' } } }, null, 2) },
    { method: 'POST', path: '/v2/capabilities', description: '统一创建能力草稿。对外只需要 source、detectedType、displayName、resourceCode 以及少量可选高级字段；底层注册协议、spec 和默认映射由后端补齐。须提供 X-User-Id。', params: [{ name: 'body', type: 'CapabilityCreateRequest', required: true, description: '统一能力注册请求' }], responseExample: JSON.stringify({ code: 0, message: 'ok', data: { capabilityId: 101, capabilityType: 'mcp', displayName: 'Weather Tools', resourceCode: 'weather-tools', status: 'draft' } }, null, 2) },
    { method: 'GET', path: '/v2/capabilities', description: '统一列出已发布能力。仍保留 skill / mcp / agent 市场分类，但对外消费视角统一为 capability。', params: [{ name: 'keyword', type: 'string', required: false, description: '关键字' }, { name: 'capabilityType', type: 'string', required: false, description: 'agent / skill / mcp' }], responseExample: JSON.stringify({ code: 0, message: 'ok', data: [{ capabilityId: 1, capabilityType: 'agent', displayName: 'Course Advisor', status: 'published' }] }, null, 2) },
    { method: 'GET', path: '/v2/capabilities/{id}', description: '读取单个能力详情。返回统一的运行时信息、输入 schema、绑定与兼容接入方式，适合作为市场详情页和 SDK 生成的真源。', params: [{ name: 'id', type: 'number', required: true, description: '能力 ID' }], responseExample: JSON.stringify({ code: 0, message: 'ok', data: { capabilityId: 9, capabilityType: 'skill', displayName: 'Slides Copilot', runtimeMode: 'prompt_context', bindings: [56] } }, null, 2) },
    { method: 'POST', path: '/v2/capabilities/{id}/resolve', description: '统一解析能力，返回建议 payload、绑定闭包和底层运行时信息。建议前端先调用它来生成“最小测试输入”。支持 X-Api-Key 或已登录用户。', params: [{ name: 'id', type: 'number', required: true, description: '能力 ID' }, { name: 'include', type: 'string', required: false, description: 'closure / bindings / bindingClosure' }], responseExample: JSON.stringify({ code: 0, message: 'ok', data: { capabilityId: 9, capabilityType: 'skill', suggestedPayload: { input: 'hello' }, resolve: { invokeType: 'gateway_v2' } } }, null, 2) },
    { method: 'POST', path: '/v2/capabilities/{id}/invoke', description: '统一试用与统一对外调用入口。skill / mcp / agent 都可从这里直接消费；底层若需要 prompt、MCP tool 调度或 agent 代理，均由后台隐藏和编排。支持 X-Api-Key 或已登录用户。', params: [{ name: 'id', type: 'number', required: true, description: '能力 ID' }, { name: 'payload', type: 'object', required: false, description: '调用输入' }, { name: 'timeoutSec', type: 'number', required: false, description: '超时时间' }], responseExample: JSON.stringify({ code: 0, message: 'ok', data: { capabilityId: 1, capabilityType: 'agent', response: { statusCode: 200, body: '{\"answer\":\"ok\"}' } } }, null, 2) },
    { method: 'POST', path: '/v2/capabilities/{id}/tool-session', description: '统一工具测试入口。前端默认只暴露“连接服务”“列出工具”“运行工具”，不再要求用户手写 initialize / tools/list / tools/call 等 JSON-RPC。支持 X-Api-Key 或已登录用户。', params: [{ name: 'id', type: 'number', required: true, description: '能力 ID' }, { name: 'action', type: 'string', required: true, description: 'connect / list_tools / call_tool' }, { name: 'toolName', type: 'string', required: false, description: '工具名' }, { name: 'arguments', type: 'object', required: false, description: '工具参数' }], responseExample: JSON.stringify({ code: 0, message: 'ok', data: { capabilityId: 56, tools: [{ name: 'weather.search', description: 'Search weather by city' }], warnings: [] } }, null, 2) },
  ]},
