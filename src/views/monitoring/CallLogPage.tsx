      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left text-sm min-w-[1000px]">
          <thead>
            ...
            >
              {['时间', '方法', '状态码', '状态', '延迟', 'Agent', '模型', 'Tokens'].map((h) => (
                <th key={h} className={`px-4 py-3 font-semibold whitespace-nowrap ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>
                  {h}
                </th>
              ))}
            </tr>
            ...
            {rows.map((r, i) => {
              ...
              return (
                ...
                >
                  ...
                  <td
                    className={`px-4 py-3 text-xs max-w-[140px] truncate ${isDark ? 'text-slate-300' : 'text-slate-700'}`}
                    title={r.agentName}
                  >
                    {r.agentName}
                  </td>
                  <td
                    className={`px-4 py-3 text-xs max-w-[140px] truncate ${isDark ? 'text-slate-400' : 'text-slate-600'}`}
                    title={r.model}
                  >
                    {r.model}
                  </td>
                  <td className={`px-4 py-3 text-xs tabular-nums ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
