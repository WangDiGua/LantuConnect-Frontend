...
export const HealthConfigPage: React.FC<Props> = ({ theme, fontSize, showMessage }) => {
  ...
  const toolbar = (
    <div className={TOOLBAR_ROW}>
      ...
      <select className={`${selectCls} w-full sm:w-[8rem] shrink-0`} value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
                    ...
                    {filtered.map((r, i) => {
                      ...
                      return (
                        <tr key={r.id} className={tableBodyRow(theme, i)}>
                          <td className={`${tableCell()} font-medium ${textPrimary(theme)}`}>{r.displayName}</td>
                          ...
                          <td className={`${tableCell()} text-xs font-mono max-w-[200px] truncate ${textMuted(theme)}`} title={r.checkUrl}>{r.checkUrl}</td>
      ...
      >
        <div className="space-y-4">
          <div>
            ...
            <select className={selectCls} value={draft.checkType ?? 'http'} onChange={(e) => setDraft((p) => ({ ...p, checkType: e.target.value as HealthConfigItem['checkType'] }))}>
