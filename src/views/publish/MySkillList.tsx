...
export const MySkillList: React.FC<Props> = ({ theme }) => {
  ...
  const fetchData = useCallback(() => {
    setLoading(true);
    userActivityService.getMySkills().then(data => setSkills(data)).catch(err => console.error(err)).finally(() => setLoading(false));
  }, []);
    ...
    <div className={`flex-1 flex flex-col min-h-0 overflow-hidden ${pageBg(theme)}`}>
      ...
      <div className={`shrink-0 z-20 border-b px-4 sm:px-6 py-4 flex items-center justify-between ${isDark ? 'border-white/[0.06]' : 'border-slate-100'}`}>
        <div className="flex items-center gap-3">
          <div className={`p-2 rounded-xl ${isDark ? 'bg-purple-500/15' : 'bg-purple-50'}`}><Wrench size={20} className={isDark ? 'text-purple-400' : 'text-purple-600'} /></div>
          <div>
            <h2 className={`text-lg font-bold ${textPrimary(theme)}`}>我的 Skill</h2>
            <p className={`text-xs ${textMuted(theme)}`}>管理您提交的技能，跟踪审核进度</p>
          </div>
        </div>
        <span className={`text-xs px-3 py-1.5 rounded-lg font-medium ${isDark ? 'bg-white/5 text-slate-400' : 'bg-slate-50 text-slate-500 border border-slate-100'}`}>共 {skills.length} 个</span>
      </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      <h3 className={`font-bold truncate ${textPrimary(theme)}`}>{skill.displayName}</h3>
                    ...
                    <div className="flex items-center gap-4 mt-2">
                      <span className={`text-xs ${textMuted(theme)}`}>创建时间: {skill.createTime}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <button onClick={() => setViewTarget(skill)} className={`p-2 rounded-xl transition-colors ${isDark ? 'hover:bg-white/5 text-slate-400' : 'hover:bg-slate-100 text-slate-500'}`} title="查看"><Eye size={16} /></button>
                    {skill.status === 'pending_review' && (
                      <button onClick={() => setWithdrawTarget(skill)} className={`p-2 rounded-xl transition-colors ${isDark ? 'hover:bg-amber-500/15 text-amber-400' : 'hover:bg-amber-50 text-amber-600'}`} title="撤回"><Undo2 size={16} /></button>
                    )}
                  </div>
                </div>
              </motion.div>
