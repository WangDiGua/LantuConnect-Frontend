...
import { ArrowLeft, ArrowRight, Check, Rocket, Save, AlertCircle, Info, Puzzle } from 'lucide-react';
...
export const SubmitSkill: React.FC<Props> = ({ theme, fontSize }) => {
  ...
  const validateStep1 = (): boolean => {
    ...
    if (!form.displayName.trim()) errs.displayName = '请输入 Skill 名称';
    if (form.displayName.trim().length > 50) errs.displayName = '名称不能超过 50 个字符';
    if (!form.description.trim()) errs.description = '请输入 Skill 描述';
    setErrors(errs);
        ...
        <div className="flex-1 flex items-center justify-center">
          ...
          >
            ...
            <p className={`text-sm mb-6 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
              您的 Skill「{form.displayName}」已提交审核，管理员将尽快处理。
            </p>
            <button
    ...
    <div className={`flex-1 flex flex-col min-h-0 overflow-hidden ${isDark ? 'bg-[#000000]' : 'bg-[#F2F2F7]'}`}>
      <div className={`shrink-0 z-20 border-b px-4 sm:px-6 py-4 flex items-center gap-4 ${isDark ? 'border-white/10 bg-[#000000]' : 'border-slate-200/80 bg-[#F2F2F7]'}`}>
        ...
        <div className="flex items-center gap-3">
          <div className={`p-2 rounded-xl ${isDark ? 'bg-violet-500/20' : 'bg-violet-50'}`}>
            <Puzzle size={20} className={isDark ? 'text-violet-400' : 'text-violet-600'} />
          </div>
          <div>
            <h2 className={`text-lg font-bold ${isDark ? 'text-white' : 'text-slate-900'}`}>提交 Skill</h2>
              ...
              <AnimatePresence mode="wait">
                ...
                {step === 2 && (
                  <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
                    ...
                    <div>
                      <label className={labelCls}>参数 Schema (JSON, 可选)</label>
                      <textarea
                        rows={4}
                        placeholder={'{\n  "type": "object",\n  "properties": { ... }\n}'}
                        className={`${nativeInputClass(theme)} !min-h-[100px] font-mono text-xs resize-y ${errors.parametersSchema ? '!border-red-500' : ''}`}
                    ...
                    <div className="flex justify-between pt-4">
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-medium bg-violet-500 text-white hover:bg-violet-600 transition-colors shadow-lg shadow-violet-500/20"
                      >
                        <Rocket size={16} />
                        提交审核
                      </button>
