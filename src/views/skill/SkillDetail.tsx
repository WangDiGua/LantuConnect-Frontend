...
export const SkillDetail: React.FC<Props> = ({ skillId, theme, fontSize: _fontSize, onBack }) => {
  ...
  const fetchSkill = () => { setLoading(true); setError(null); skillService.getById(numericId).then(setSkill).catch(err => setError(err instanceof Error ? err : new Error('加载失败'))).finally(() => setLoading(false)); };
  useEffect(() => { fetchSkill(); }, [numericId]);

  const handleDelete = async () => { setDeleting(true); try { await skillService.remove(numericId); setDeleteOpen(false); onBack(); } catch { setDeleting(false); } };

  if (loading) return <div className={`flex-1 flex flex-col min-h-0 ${pageBg(theme)}`}><PageSkeleton type="detail" /></div>;
  if (error || !skill) return <div className={`flex-1 flex flex-col min-h-0 ${pageBg(theme)}`}><PageError error={error} onRetry={fetchSkill} /></div>;
      ...
      <div className={`shrink-0 z-20 border-b px-4 sm:px-6 py-4 flex items-center justify-between ${isDark ? 'border-white/[0.06]' : 'border-slate-100'}`}>
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="flex items-center gap-3 min-w-0">
            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-2xl shrink-0 ${isDark ? 'bg-purple-500/15' : 'bg-purple-50'}`}>{skill.icon || '🔧'}</div>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar px-2 sm:px-3 lg:px-4 py-3 grid grid-cols-1 lg:grid-cols-3 gap-4 content-start">
        ...
        <div className="lg:col-span-2 space-y-4">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 300, damping: 30 }} className={`${bentoCard(theme)} p-6`}>
            ...
            <div className="grid grid-cols-2 gap-5">
              {[
                { label: '显示名称', value: skill.displayName }, { label: '标识名称', value: skill.agentName, mono: true },
                { label: '协议类型', value: AGENT_TYPE_LABEL[skill.agentType] ?? skill.agentType }, { label: '来源', value: SOURCE_TYPE_LABEL[skill.sourceType] ?? skill.sourceType },
                { label: '所属 MCP Server', value: skill.parentName ?? '独立 Skill' }, { label: '展示模板', value: skill.displayTemplate ?? '无' },
                { label: '分类', value: skill.categoryName ?? '未分类' }, { label: '公开', value: skill.isPublic ? '是' : '否' },
              ].map((item) => (
                <div key={item.label}><label className={`text-xs block mb-1 ${textMuted(theme)}`}>{item.label}</label><p className={`font-medium ${item.mono ? 'font-mono text-sm' : ''} ${textPrimary(theme)}`}>{item.value}</p></div>
              ))}
              <div className="col-span-2"><label className={`text-xs block mb-1 ${textMuted(theme)}`}>描述</label><p className={`text-sm leading-relaxed ${textSecondary(theme)}`}>{skill.description}</p></div>
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 300, damping: 30, delay: 0.05 }} className={`${bentoCard(theme)} p-6`}>
            <h3 className={`text-sm font-bold mb-4 flex items-center gap-2 ${textPrimary(theme)}`}><Settings size={18} className="text-purple-500" /> 连接配置</h3>
            <div className="space-y-3">
              ...
              {specJson.api_key && <div className={`flex items-start gap-3 p-3 rounded-xl ${isDark ? 'bg-white/[0.02]' : 'bg-slate-50'} border ${isDark ? 'border-white/[0.06]' : 'border-slate-100'}`}><div className={`p-2 rounded-xl shrink-0 ${isDark ? 'bg-amber-500/15 text-amber-400' : 'bg-amber-50 text-amber-600'}`}><Settings size={18} /></div><div className="min-w-0"><p className={`text-sm font-bold mb-1 ${textPrimary(theme)}`}>API Key</p><p className={`text-xs font-mono ${textMuted(theme)}`}>••••••••</p></div></div>}
              <div className={`flex items-start gap-3 p-3 rounded-xl ${isDark ? 'bg-white/[0.02]' : 'bg-slate-50'} border ${isDark ? 'border-white/[0.06]' : 'border-slate-100'}`}>
              ...
              {skill.parentId && <div className={`flex items-start gap-3 p-3 rounded-xl ${isDark ? 'bg-white/[0.02]' : 'bg-slate-50'} border ${isDark ? 'border-white/[0.06]' : 'border-slate-100'}`}><div className={`p-2 rounded-xl shrink-0 ${isDark ? 'bg-violet-500/15 text-violet-400' : 'bg-violet-50 text-violet-600'}`}><Server size={18} /></div><div className="min-w-0"><p className={`text-sm font-bold mb-1 ${textPrimary(theme)}`}>MCP Server</p><p className={`text-xs ${textMuted(theme)}`}>{skill.parentName ?? `ID: ${skill.parentId}`}</p></div></div>}
            </div>
          </motion.div>
        ...
        <div className="space-y-4">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 300, damping: 30, delay: 0.1 }} className={`${bentoCard(theme)} p-6`}>
            ...
            <div className="space-y-5">
              {[
                ...
                { label: '质量评分', value: String(skill.qualityScore), color: 'text-purple-500' },
              ].map((item) => (
                <div key={item.label}>
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ type: 'spring', stiffness: 300, damping: 30, delay: 0.15 }} className={`${bentoCard(theme)} p-6`}>
            ...
            <div className="space-y-3">
              <div className="flex justify-between text-xs"><span className={textMuted(theme)}>创建时间</span><span className={`font-mono ${textSecondary(theme)}`}>{skill.createTime}</span></div>
              <div className="flex justify-between text-xs"><span className={textMuted(theme)}>最后更新</span><span className={`font-mono ${textSecondary(theme)}`}>{skill.updateTime}</span></div>
            </div>
          </motion.div>
        </div>
      </div>

      <ConfirmDialog open={deleteOpen} title="删除 Skill" message={`确定要删除「${skill.displayName}」吗？此操作不可撤销。`} confirmText="删除" variant="danger" loading={deleting} onConfirm={handleDelete} onCancel={() => setDeleteOpen(false)} />
    </div>
  );
};
