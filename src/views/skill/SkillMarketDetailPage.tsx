  ...
  return (
    <>
            ...
            {tab === 'intro' ? (
              <div className="space-y-4">
                ...
                <div className={`flex flex-wrap gap-3 text-xs ${textMuted(theme)}`}>
                  ...
                  <span className="text-fuchsia-600 dark:text-fuchsia-300">Context 规范 · POST /catalog/resolve</span>
              ...
              <div className="space-y-4">
                <p className={`text-sm leading-relaxed ${textSecondary(theme)}`}>
                  技能为 Context 规范：目录 spec 中含 <span className="font-mono text-xs">executionMode=context</span>、
                  <span className="font-mono text-xs">contextPrompt</span>、
                  <span className="font-mono text-xs">parametersSchema</span> 等。请使用{' '}
                  <span className="font-mono text-xs">POST /catalog/resolve</span> 获取完整解析结果；不可{' '}
                  <span className="font-mono text-xs">POST /invoke</span>（resourceType=skill）。
                </p>
                <div
      ...
      <Modal
        ...
        title={`POST /catalog/resolve — ${skill.displayName}`}
        theme={theme}
        size="md"
        footer={(
          <>
      ...
      >
        <div className="space-y-4">
          ...
          <p className={`text-xs leading-relaxed ${textMuted(theme)}`}>
            点击下方按钮调用 <span className="font-mono">POST /catalog/resolve</span>，返回 <span className="font-mono">invokeType</span>、
            <span className="font-mono">spec</span> 与可选绑定闭包。
          </p>
