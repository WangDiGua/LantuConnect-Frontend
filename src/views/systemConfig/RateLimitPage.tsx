      ...
      <BentoCard theme={theme} padding="md" className="mx-4 sm:mx-6 mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-3xl">
          ...
          <div>
            ...
            <select className={selectCls} value={draft.target} onChange={(e) => setDraft((d) => ({ ...d, target: e.target.value as any }))}>
          ...
          <div>
            ...
            <select className={selectCls} value={draft.action} onChange={(e) => setDraft((d) => ({ ...d, action: e.target.value as any }))}>
                      ...
                      <tr key={r.id} className={tableBodyRow(theme, i)}>
                        ...
                        <td className={`${tableCell()} text-right`}>
                          <div className="inline-flex gap-1 justify-end">
                            <button type="button" onClick={() => startEdit(r)} className={`px-2 py-1 rounded-xl text-xs font-medium ${isDark ? 'text-indigo-400 hover:bg-white/10' : 'text-indigo-600 hover:bg-indigo-50'}`}>编辑</button>
