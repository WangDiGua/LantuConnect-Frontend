...
export const OrgStructurePage: React.FC<OrgStructurePageProps> = ({ theme }) => {
  ...
  const rows = data.flatMap((root) => flattenOrg(root));
        ...
        <div className="flex min-w-0 items-center justify-between gap-3">
          ...
          <div className="shrink-0">
            <button type="button" onClick={async () => {
              ...
              if (!name?.trim()) return;
              try {
                await userMgmtService.createOrg({ name: name.trim(), parentId: primaryRoot?.id });
                refetch();
              } catch (err) {
              ...
              {rows.map((d, rowIdx) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    ...
                    <button type="button" onClick={async () => {
                      const newName = prompt('修改部门名称', d.name);
                      if (!newName?.trim() || newName === d.name) return;
                      try {
                        await userMgmtService.updateOrg(d.id, { name: newName.trim() });
                        refetch();
                      } catch {}
                    }} className={`text-xs ${textMuted(theme)} hover:text-neutral-800`}>编辑</button>
                    <button type="button" onClick={async () => {
                    ...
                    }} className={`text-xs text-rose-500 hover:text-rose-600`}>删除</button>
                  </div>
                </li>
              ))}
            </ul>
