...
import { Search, Plus, ArrowLeft, Save, Trash2, Fingerprint, Loader2 } from 'lucide-react';
...
export const RoleListPage: React.FC<RoleListPageProps> = ({ theme, fontSize, breadcrumbBase }) => {
  ...
  const fetchRoles = useCallback(async () => {
    setLoading(true);
    try {
      const result = await userMgmtService.listRoles();
      setRoles(result);
    } catch (err) {
  ...
  const saveRole = async () => {
    ...
    try {
      if (mode === 'create') {
        await userMgmtService.createRole({
          name: form.name.trim(),
        ...
        await userMgmtService.updateRole(editingId, {
          name: form.name.trim(),
          code: form.code.trim().replace(/\s+/g, '_').toLowerCase(),
          description: form.description.trim(),
          permissions,
    ...
    mode === 'list' ? (
      <div className={TOOLBAR_ROW}>
        <div className="relative flex-1 min-w-0 sm:max-w-md">
          ...
          <input type="search" placeholder="搜索名称、代码或描述…" value={search} onChange={(e) => setSearch(e.target.value)} className={searchCls} />
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button type="button" onClick={saveRole} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-medium bg-blue-600 text-white hover:bg-blue-700">
          <Save size={16} />
          保存
        </button>
      </div>
  ...
  return (
    <MgmtPageShell theme={theme} fontSize={fontSize} breadcrumbSegments={breadcrumbSegments} titleIcon={Fingerprint} description={description} toolbar={toolbar}>
      <div className="relative min-w-0">
        <AnimatePresence mode="wait">
          {mode === 'list' ? (
            <motion.div key="role-list" initial={{ opacity: 0, x: 14 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} transition={innerTransition} className="w-full px-4 sm:px-6 pb-6 pt-1 min-w-0">
              {loading && roles.length === 0 ? (
                ...
                <>
                    ...
                    <table className="w-full text-left text-sm min-w-[800px]">
                      <thead>
                        <tr className={`border-b ${isDark ? 'border-white/10 bg-[#2C2C2E]/80' : 'border-slate-200 bg-slate-50'}`}>
                          ...
                          <th className={`px-4 py-3 font-semibold ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>代码</th>
                          <th className={`px-4 py-3 font-semibold ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>描述</th>
                          <th className={`px-4 py-3 font-semibold ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>用户数</th>
                          <th className={`px-4 py-3 font-semibold ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>权限摘要</th>
                      ...
                      <tbody>
                        {paginated.map((r, i) => (
                          <tr key={r.id} className={`border-b transition-colors ${isDark ? `border-white/5 ${i % 2 === 0 ? 'bg-[#1C1C1E]' : 'bg-[#2C2C2E]/40'} hover:bg-white/5` : `border-slate-100 ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/80'} hover:bg-slate-100/80`}`}>
                            ...
                            <td className={`px-4 py-3 font-mono text-xs ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>{r.code}</td>
                            <td className={`px-4 py-3 max-w-[200px] truncate ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>{r.description}</td>
                            <td className={`px-4 py-3 ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>{r.userCount}</td>
                            <td className={`px-4 py-3 text-xs font-mono max-w-[220px] truncate ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>{r.permissions.join(', ')}</td>
                            <td className="px-4 py-3 text-right">
                              <div className="inline-flex items-center gap-1">
                                ...
                                <button type="button" onClick={() => setDeleteTarget(r.id)} className={`p-1 rounded-xl ${isDark ? 'text-red-400 hover:bg-white/10' : 'text-red-600 hover:bg-red-50'}`} title="删除"><Trash2 size={16} /></button>
                              </div>
                            </td>
                          </tr>
                        ))}
                  ...
                  {filtered.length === 0 && <p className={`text-center py-12 text-sm ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>暂无匹配角色</p>}
                  {filtered.length > 0 && <Pagination page={page} pageSize={PAGE_SIZE} total={filtered.length} onChange={setPage} />}
                </>
              )}
            </motion.div>
          ) : (
            <motion.div key="role-form" initial={{ opacity: 0, x: 14 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} transition={innerTransition} className="w-full px-4 sm:px-6 pb-6 pt-1 max-w-2xl min-w-0">
              <div className="space-y-4">
                <div>
                  <label className={labelCls}>角色名称</label>
                  <input className={inputCls} value={form.name} onChange={(e) => setForm((f) => ({ ...f, name: e.target.value }))} placeholder="如：学科助理" />
                </div>
                <div>
                  <label className={labelCls}>角色代码</label>
                  <input className={inputCls} value={form.code} onChange={(e) => setForm((f) => ({ ...f, code: e.target.value }))} placeholder="英文标识，如 subject_assistant" />
                </div>
                <div>
                  <label className={labelCls}>描述</label>
                  <textarea rows={3} className={`${inputCls} min-h-[5.5rem]`} value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} placeholder="简要说明该角色的使用场景" />
                </div>
                <div>
                  <span className={labelCls}>权限分组</span>
                  <div className={`rounded-xl border p-3 space-y-2 ${isDark ? 'border-white/10 bg-[#2C2C2E]/50' : 'border-slate-200 bg-slate-50/80'}`}>
                    {PERMISSION_PRESETS.map((p) => (
                      <label key={p.id} className={`flex items-center gap-3 cursor-pointer rounded-lg px-2 py-2 ${isDark ? 'hover:bg-white/5' : 'hover:bg-white'}`}>
                        <input type="checkbox" checked={form.presetIds.has(p.id)} onChange={() => togglePreset(p.id)} className="rounded border-slate-300 text-blue-600 focus:ring-blue-500" />
                        <span className={`text-sm ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>{p.label}</span>
                        <span className={`text-xs ml-auto font-mono ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>{p.id}:*</span>
                      </label>
                    ))}
                  </div>
                </div>
